...
public class ScoreInterfaceProcessor extends AbstractProcessor {
    ...
    public synchronized void init(ProcessingEnvironment processingEnv) {
        ...
        util = new ProcessorUtil(processingEnv, ScoreInterfaceProcessor.class.getSimpleName());
    }
        ...
        for (TypeElement annotation : annotations) {
            ...
            for (Element element : annotationElements) {
                if (element.getKind().isInterface() || element.getKind().isClass()) {
                    util.noteMessage("process %s %s", element.getKind(), element.asType(), element.getSimpleName());
    ...
    private List<MethodSpec> overrideMethods(TypeElement element) {
        ...
        if (!superClass.getKind().equals(TypeKind.NONE) && !superClass.toString().equals(Object.class.getName())) {
            util.noteMessage("superClass[kind:%s, name:%s]", superClass.getKind().name(), superClass.toString());
            List<MethodSpec> superMethods = overrideMethods(util.getTypeElement(element.getSuperclass()));
