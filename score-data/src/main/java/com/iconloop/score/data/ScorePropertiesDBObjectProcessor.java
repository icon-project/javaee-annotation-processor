...
import com.iconloop.annotation_processor.AnnotatedTypeElement;
...
public class ScorePropertiesDBObjectProcessor extends AbstractProcessor {
    ...
    public synchronized void init(ProcessingEnvironment processingEnv) {
        ...
        for (Class<?> clazz : BytesCodec.predefinedCodecs.keySet()) {
            bytesCodecSupportedTypes.put(clazz, super.getTypeMirror(clazz));
        }
        ...
        for (Element enclosedElement : element.getEnclosedElements()) {
            if (enclosedElement.getKind().equals(ElementKind.FIELD) &&
                ...
                if (annField != null) {
                    ...
                    TypeMirror t = super.getTypeMirrorFromAnnotation(annField::db);
                ...
                if (super.containsDeclaredType(dbConstructors.keySet(), fieldType)) {
                    ...
                    if (arrayDBValueType.getKind().isPrimitive()) {
                        arrayDBValueType = super.getBoxedType(arrayDBValueType);
                    }
                    closeMethod.addStatement("super.$L(null)", setter);
                    ...
                    String defaultVal = ProcessorUtil.getDefaultValueAsString(fieldType);
                    if (fieldType.getKind().isPrimitive()) {
                        codecType = super.getBoxedType(fieldType);
                    ...
                    if (typeUtil.isSameType(fieldType, bytesType)) {
                        getterCodeBlock
                                .addStatement("$T $L = \"$L\"", String.class, localKey, key)
                    ...
                    if (dbClass == null) {
                        AnnotatedTypeElement<ScorePropertiesDBObject> annotated =
                                super.getAnnotatedTypeElement(fieldType, ScorePropertiesDBObject.class);
                        if (annotated != null) {
                    ...
                    if (fieldClassName != null) {
                        ...
                        String readMethod = findReadMethod(fieldType);
                        String writeMethod = findWriteMethod(fieldType);
